...
**Server actions can't take a Prisma client as a parameter.** Every exported function in a `"use server"` file is called from client components via RPC, so its arguments must be serializable — a Prisma client instance can't cross that boundary. This means the `BoardDb`/`ExportDb`-style injectable-client pattern above doesn't work for files like [board.ts](src/server/actions/board.ts) directly. If board's mutations need to become unit-testable, the shape is: extract the DB-touching logic into a plain (non-`"use server"`) module that takes an injectable client, the same way `services/export.ts` is structured, and have the thin `"use server"` action wrapper call it with the real `db`.
