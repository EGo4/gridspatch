...
| # | Item | Size | Why here |
| --- | --- | --- | --- |
| 1 | [Finish splitting BoardClient.tsx](#1-boardclienttsx-is-2434-lines) | L | Do before the roadmap's board features land here |
...
## 1. BoardClient.tsx is 2434 lines
...
**Safety net status.** [test/board-mutations.test.ts](test/board-mutations.test.ts) covers the *server-action* mutation logic. A component-test harness now also exists — Vitest + jsdom + React Testing Library, see the "Component tests run on a second, separate test runner" note in [CLAUDE.md](CLAUDE.md) — with a first pass in [test/components/BoardClient.test.tsx](test/components/BoardClient.test.tsx) covering: initial pool placement, mark-sick removal, split→merge round-trip, and the syncPendingRef/skippedRebuildRef rebuild-skip behavior called out above.

**What that first pass does *not* cover yet — extend it before or during the split, not after:** actual drag-and-drop (only button-triggered actions are exercised so far; `@hello-pangea/dnd` drag simulation in jsdom needs its own investigation), copy day / copy previous week (including the overwrite-confirmation modal), both holiday types, status transitions and their confirmation modals, the past-week confirmation modal itself, and comments. Treat the four existing tests as a proof that the harness works, not as full coverage — budget time to round these out before trusting the refactor to the suite alone. Manual QA of the same list is still the honest fallback for whatever the suite ends up not reaching.
