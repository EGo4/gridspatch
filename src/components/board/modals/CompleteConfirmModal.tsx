// src/components/board/modals/CompleteConfirmModal.tsx
//
// Warns before transitioning a site into a "completed" super-status
// (see ALLOWED_TRANSITIONS / getSuperStatus in ~/types) while it still has
...
type CompleteConfirmModalProps = {
  transition: { projectId: string; status: ProjectStatus; assignmentCount: number } | null;
...
export function CompleteConfirmModal({ transition, projectName, applying, onCancel, onConfirm }: CompleteConfirmModalProps) {
      ...
      >
        <div className="flex items-center justify-between border-b border-[var(--color-border-subtle)] px-5 py-4">
          <h3 className="text-sm font-semibold text-[var(--color-text-primary)]">
            {t("markAsTitle", { status: tStatus(transition.status) })}
          </h3>
        ...
        <div className="px-5 py-4 flex flex-col gap-3">
          <p className="text-sm text-[var(--color-text-secondary)]">
            {t("completeBody", { name: projectName ?? "" })}
          </p>
          {transition.assignmentCount > 0 && (
            <div className="flex items-start gap-2 rounded-lg border border-[var(--color-warn-border)] bg-[var(--color-warn-bg)] px-3 py-2.5 text-xs text-[var(--color-warn-text)]">
              ...
              {t("completeWarn", { count: transition.assignmentCount })}
            </div>
          )}
        </div>
